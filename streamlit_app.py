...
# ゲーム画面
    ...
    # 円形ボタンのHTML生成 - 円の内側に均等な余白で配置
    button_html = ''.join([
        f'''
        <div class="circle-button" id="button_{i}"
                data-letter="{letter}"
                data-index="{i}"
                style="left: {110 + 75 * math.cos(2 * math.pi * i / num_letters - math.pi/2) - 15}px;
                       top:  {110 + 75 * math.sin(2 * math.pi * i / num_letters - math.pi/2) - 15}px;">
            {letter}
        </div>
